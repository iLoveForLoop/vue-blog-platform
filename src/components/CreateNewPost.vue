...
<template>
  <div class="back d-flex flex-column justify-content-center align-items-center poppins-regular"
    @mousedown.self="closeCreatePost">
    <transition name="slide">
      <div class="alert alert-danger errpos" role="alert" v-if="error" style="z-index: 11">
        {{ error }}
      </div>
    </transition>
    <div class="text-light text-center p-3 bg-gray border-b rounded-top-4" style="width: 40%">
      <p class="m-0">Create a new post</p>
    </div>
    <div class="rounded-bottom-4 bg-gray d-flex" style="height: 65vh; width: 40%" v-if="user">
      <form @submit.prevent="handleSubmit" class="d-flex flex-column justify-content-between w-100 p-3 pb-4">
        <div class="d-flex flex-column flex-grow-1 w-100">
          <div class="d-flex justify-content-start align-items-center gap-3">
            <img class="create-circle" :src="user.value?.photoURL
              ? user.value?.photoURL
          ...
          <textarea class="form-control w-100 bg-transparent no-border my-border text-light px-3 mt-3 flex-grow-1"
            v-model="post" ref="myTextArea" placeholder="What's new today?"></textarea>
        </div>
