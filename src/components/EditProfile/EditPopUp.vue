...
onMounted(() => {
  myTextArea.value.addEventListener('keydown', closeOnEscape)
  nextTick(() => {
...
<template>
  <div class="backdrop d-flex justify-content-center align-items-center poppins-regular" @mousedown.self="closePopOver">
    ...
    <div class="w-50 main-bg rounded-5" style="height: 40vh">
      <div class="d-flex justify-content-between p-4 align-items-center border-b">
        <p class="text-light m-0" @click="closePopOver" style="cursor: pointer">
          Cancel
        </p>
      ...
      <div>
        <textarea ref="myTextArea" v-model="text" class="form-control bg-transparent no-border px-3 mt-3 text-secondary"
          :maxlength="type == 'bio' ? '50' : '29'" @keydown.enter.prevent="sendData"></textarea>
      </div>
    </div>
  </div>
</template>
